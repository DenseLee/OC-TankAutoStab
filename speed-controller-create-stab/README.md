...
The turret controller uses a hybrid loop: quaternion position error supplies the world-space hold correction, while `ship.getOmega()` supplies direct angular-velocity and acceleration damping. `yawOmegaToRPM` and `pitchOmegaToRPM` begin at the 1:1 rad/s-to-RPM conversion and must be tuned for your turret gearing. `omegaLookAheadSeconds` controls acceleration prediction. `maxRPMChangePerSecond` controls command smoothness; `maxYawRPM` and `maxPitchRPM` remain the absolute output limits. While aiming, `manualAimMaxYawRPM` and `manualAimMaxPitchRPM` add direct RPM feed-forward; tune them to match the selected `aimDegreesPerSecond` and your turret gearing. By default, `captureTargetOnAimRelease` re-captures the actual turret orientation when both aim axes are released, preventing a return to a lagging virtual target.
