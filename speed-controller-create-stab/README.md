...
The turret controller uses a filtered PD loop: it keeps four ticks of yaw/pitch error history, filters the error rate, and smoothly slews the signed RPM command. `maxRPMChangePerSecond` controls command smoothness; `derivativeGainRPMPerDegreePerSecond` controls braking as the turret closes on its target. `maxYawRPM` and `maxPitchRPM` remain the absolute output limits. While aiming, `manualAimMaxYawRPM` and `manualAimMaxPitchRPM` add direct RPM feed-forward; tune them to match the selected `aimDegreesPerSecond` and your turret gearing.
