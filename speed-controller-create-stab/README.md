...
Run `turret_speed_controller_stab.lua` on the turret computer. Its front wireless modem sends signed yaw/pitch RPM commands to the hull computer's bottom wireless modem. The default aim relays are `redstone_relay_1` for yaw and `redstone_relay_0` for pitch; both use top/bottom for X+/X- and Y+/Y-. Steering uses `redstone_relay_2`: top is steering X+ and bottom is steering X-.

The turret controller uses an adaptive rate loop. Quaternion error requests a desired turret rate through `stabilizerResponseSeconds`; `ship.getOmega()` measures the actual rate; the controller learns `degrees/second/RPM` during steady manual movement and uses that estimate to select RPM. `rateFeedbackGain` corrects mismatch between desired and measured rate. `maxRPMChangePerSecond` controls command smoothness; `maxYawRPM` and `maxPitchRPM` remain absolute output limits. Manual input first moves a separate aim target, and `aimTargetResponseSeconds` smooths that target before the fast stabilizer tracks it. `manualAimUsesFeedforward` is disabled by default; enable it only if direct manual RPM is preferred. The default `aimInputCurveExponent` of `2.5` makes low analogue inputs slower for precision. By default, `captureTargetOnAimRelease` re-captures the actual turret orientation when both aim axes are released, preventing a return to a lagging virtual target. `hardStopInsideDeadzone` is currently enabled to test an immediate zero-RPM settle inside the deadzone after aim release.
